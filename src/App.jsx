...
export default function App() {
  ...
  const hasCompletedMonth = useMemo(() => {
    const now = new Date()
  ...
  return (
    <div className="app">
      ...
      <nav className="tab-nav">
        ...
        {hasCompletedMonth && (
          <button className={`tab-btn ${activeTab === 'calendrier' ? 'active' : ''}`} onClick={() => setActiveTab('calendrier')}>
            📅 Calendrier
          </button>
        )}
      </nav>
      ...
      {activeTab === 'calendrier' && (
        <DelayCalendarTab rounds={rounds} />
      )}
    </div>
  )
}
...
function DelayCalendarTab({ rounds }) {
  ...
  if (months.length === 0) {
    return (
      <div className="empty-state">
        <div className="empty-state-icon">📅</div>
        <h3>Pas encore de mois archivé</h3>
        <p>Le calendrier des retards apparaîtra ici une fois qu'un mois complet sera passé.</p>
      </div>
    )
  }
  ...
  const monthLabel = new Date(current.year, current.month, 1).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })

  return (
    <div className="calendar-tab-view">
      <h2 className="section-title">📅 Calendrier des Retards</h2>
      <div className="calendar-nav" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
        <button className="btn btn-secondary" onClick={() => setIndex((i) => Math.max(0, i - 1))} disabled={index === 0}>← Mois précédent</button>
        <p style={{ margin: 0, fontWeight: 600 }}>{monthLabel}</p>
        <button className="btn btn-secondary" onClick={() => setIndex((i) => Math.min(months.length - 1, i + 1))} disabled={index === months.length - 1}>Mois suivant →</button>
      </div>
      ...
      <div className="modal rules-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rules-section">
          ...
          <ul className="rules-list">
            ...
            <li><strong>Coffre aux exploits</strong> : débloque des trophées selon les réponses données par chaque participant (Pile à l'heure, Prophète du café, etc.).</li>
          </ul>
        </div>
